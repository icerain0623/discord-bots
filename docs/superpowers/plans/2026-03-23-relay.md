...
async function handleRelayModal(interaction, env, userId) {
  ...
  // getDisplayName は modals.js の先頭で既にインポート済み
...
**注意**: `getDisplayName` と `getUserId` は modals.js の先頭で既にインポート済みなのでそのまま使用する。`getRelay`, `saveRelay`, `editMessage` は relay 固有なので動的 import で取得する（matchup パターンと同様）。
