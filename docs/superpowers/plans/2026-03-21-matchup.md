...
export async function postMessage(channelId, token, payload) {
  // payload can be a string (content only) or an object (embeds, components, etc.)
...
export async function sendFollowupMessage(applicationId, interactionToken, payload) {
  // Like sendFollowup but supports flags (e.g. ephemeral) and arbitrary payload
...
async function doStart(kv, guildId, groupSize, categoryId, channelId, applicationId, interactionToken, env) {
  ...
  // Post recruitment message using postMessage with rich payload
...
> **Note:** The `handleStart` signature already includes `ctx` (fixed in Task 5's stub). The call site in `handleMatchup` also already passes `ctx`.
...
- [ ] **Step 5: Write tests for participation flow**

Create `tests/matchupParticipation.test.js`:
...
describe('matchup_join button', () => {
  ...
  test('shows select menu when topics exist', async () => {
    ...
    const env = { SESSION_KV: sessionKV, MATCHUP_KV: matchupKV }
    const result = await handleButton(makeButtonInteraction('matchup_join'), env)
    // Should contain a string select component (type 3)
...
```bash
npm test -- tests/matchupParticipation.test.js
```
...
```bash
git add src/interactions/buttons.js src/interactions/modals.js src/modals/matchupFreeTopics.js tests/matchupParticipation.test.js
git commit -m "feat(matchup): add participation flow (button, select, modal) with tests"
```
...
First, add `ChannelType` to the import in `src/deploy-commands.js`:
```javascript
import { REST, Routes, SlashCommandBuilder, PermissionFlagsBits, ChannelType } from 'discord.js'
```

Then add to the `commands` array:
...
### Task 13: Version bump & documentation update

**Files:**
- Modify: `package.json` (version bump)
- Modify: `README.md` (add matchup command docs)
- Modify: `RELEASE_NOTES.md` (add changelog entry)

- [ ] **Step 1: Bump version in package.json**

Update version from `0.6.0` to `0.7.0` (new feature).

- [ ] **Step 2: Update README.md**

Add `/matchup` command documentation to the command list section, including subcommands and usage examples.

- [ ] **Step 3: Update RELEASE_NOTES.md**

Add v0.7.0 entry describing the matchup feature.

- [ ] **Step 4: Update status.js version**

Update the `VERSION` constant in `src/commands/status.js` to `'0.7.0'`.

- [ ] **Step 5: Commit**

```bash
git add package.json README.md RELEASE_NOTES.md src/commands/status.js
git commit -m "docs: update docs and bump version to v0.7.0 for matchup feature"
```
