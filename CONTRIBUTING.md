...
Bot の動作がおかしいときは、GitHub Issue で報告してください。
「これってバグかな？」くらいの段階でも気軽に投稿して大丈夫です。
...
- `[Bug]` が自動で付きます。その後ろに **何が起きたかをひと言で** 書いてください
- 良い例: `[Bug] /emoji-stats で「今週」を選ぶとエラーになる`
- 悪い例: `[Bug] 動かない`
...
| セクション | 書くこと | 例 |
|---|---|---|
| **何が起きたか** | 問題の概要をひと言で | 「/emoji-stats コマンドで『今週』を選択するとエラーメッセージが表示される」 |
| **再現手順** | バグを再現する手順を番号付きで | 「1. /emoji-stats を実行 → 2. 期間で『今週』を選択 → 3. エラーが表示される」 |
| **期待する動作** | 本来どう動くべきか | 「今週の絵文字ランキングが表示される」 |
| **実際の動作** | 実際にどうなったか | 「『インタラクションに失敗しました』と表示される」 |
| **環境** | サーバー名・デバイス・発生日時 | 「PC、3/21 15:00頃」 |
| **スクリーンショット** | あれば貼る（なければ省略 OK） | ドラッグ&ドロップで貼り付け |

#### Tips

- **エラーメッセージはそのままコピペ** — 自分の言葉で言い換えず、表示されたメッセージをそのまま貼ってください
- **「毎回起きる」か「たまに起きる」か** を書いてもらえると調査しやすいです
- **スクリーンショット大歓迎** — 文章で伝えにくいときは画像を貼ってください
